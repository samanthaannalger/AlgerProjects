...
BombSurv2 <- read.table("BombSurv2.csv",header=TRUE,sep=",",stringsAsFactors=FALSE)
...
# remove unneeded columns from the DF
BombSurv2 <- select(BombSurv2, -City, -Name, -virusBINY_PreFilter)
...
BombSurv$Date_Pooled <- ifelse(BombSurv$Date_collected >= "2015-08-03", 90, ifelse(BombSurv$Date_collected >= "2015-07-28", 67, ifelse(BombSurv$Date_collected >= "2015-07-16", 61, 27))) 
...
library(plyr)
VirusSum2 <- ddply(sp$Bimaculatus, c("target_name", "Date_collected"), summarise, 
                  n = length(virusBINY),
                  mean = mean(virusBINY),
                  sd = sd(virusBINY),
                  se = sd / sqrt(n))

VirusSum2 <- VirusSum2[-(31:44),]
...
plot <- ggplot(data = VirusSum2, 
               aes(x = Date_collected, 
                   ...
                   group = target_name, 
                   colour = target_name)
               ) + geom_line(size=1.5) + geom_point(size=4) + scale_colour_manual(values = c("dodgerblue4", "black")) + coord_cartesian(ylim = c(0, 1)) + labs(x = "Time", y = "% Prevalence", color="Virus:") + geom_errorbar(aes(ymin = mean - se, ymax = mean + se, width = 1.5)) + scale_y_continuous(labels = scales::percent) 

# add a theme and add asterix for significance 
plot + scale_fill_brewer(palette = "Paired") + theme_minimal(base_size = 17) + theme(legend.position=c(.85, .85)) + coord_cartesian(ylim = c(0, 1), xlim=c(25,100))
...
CopDist <- ddply(BombSurv2, c("target_name", "site"), summarise, 
