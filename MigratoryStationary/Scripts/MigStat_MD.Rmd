...
MigStat$NosemaLoadRecount <- log(MigStat$NosemaLoadRecount+1)
#MigStat$NosemaLoadRecount[MigStat$NosemaLoadRecount=="Inf"] <- 0
...
Fullmod <- glmer(data=MigStatExp_1, formula = DWVbinary~Treatment * SamplingEvent + (1|ID) + (1|Yard), family = binomial(link = "logit"))
Anova(Fullmod)
...
nbDWV <- lmer(data=MigStatExp_1, formula = logDWV~Treatment * SamplingEvent + (1|ID)+ (1|Yard))
Anova(nbDWV)
...
nbBQCV <- lmer(data=MigStatExp_1, formula = logBQCV~Treatment * SamplingEvent + (1|ID)+ (1|Yard))
Anova(nbBQCV)
...
Fullmod2 <- glmer(data=MigStatExp_1, formula = VarroaBinary~Treatment * SamplingEvent + (1|ID)+ (1|Yard), family = binomial(link = "logit"))
Anova(Fullmod2)
```
...
nbVAR <- lmer(data=MigStatExp_1, formula = Varroa~Treatment * SamplingEvent + (1|ID)+ (1|Yard))
Anova(nbVAR)
```
...
Mig2 <- Mig2[!Mig2$Treatment=="Exposed",]

# run LDA
time2 <- lda(Treatment~Varroa + NosemaLoadRecount + logBQCV + logDWV + DWVbinary + FOB + NosemaBinary + VarroaBinary, data=Mig2, na.action="na.omit")
...
CA1 <- lda(Treatment~Varroa + NosemaLoadRecount + logBQCV + logDWV + DWVbinary + FOB + NosemaBinary + VarroaBinary, data=Mig2, na.action="na.omit",prior=c(1,1)/2, CV=TRUE)

# create confusion matrix
cm1 <- data.frame(cbind(Mig2$Treatment, CA1$class))
names(cm1) <- c("actual", "predicted")
cm <- table(cm1)
colnames(cm) = c('Migratory', 'Stationary')
rownames(cm) = c('Migratory', 'Stationary')
cm
...
Mig2x <- na.omit(Mig2)
x <- dplyr::select(Mig2x, Varroa, NosemaLoadRecount, logBQCV, logDWV, DWVbinary, BSB, FOB, NosemaBinary, VarroaBinary)
...
# run LDA
time3 <- lda(Treatment~Varroa + NosemaLoadRecount + logBQCV + logDWV + DWVbinary + FOB + NosemaBinary + VarroaBinary, data=Mig3, na.action="na.omit")
...
CA2 <- lda(Treatment~Varroa + NosemaLoadRecount + logBQCV + logDWV + DWVbinary + FOB + NosemaBinary + VarroaBinary, data=Mig3, na.action="na.omit",prior=c(1,1,1)/3, CV=TRUE)
...
Mig3x <- na.omit(Mig3)
x1 <- dplyr::select(Mig3x,Varroa, NosemaLoadRecount, logBQCV, logDWV, DWVbinary, FOB, NosemaBinary, VarroaBinary)
